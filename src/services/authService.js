...
const Login = (type, { username, password }) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (!user) {
      const err = constants.errors.INVALID_ARGS;
...
const Register = (type, { name, surname, address, email, phoneNumber, serviceType, username, password }) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (modelName == constants.AUTH_TYPES.CONSUMER && !surname) {
      return reject(constants.errors.MISSING_ARGS);
    } else if (modelName == constants.AUTH_TYPES.RESTAURANT && (!address || !serviceType)) {
      return reject(constants.errors.MISSING_ARGS);
    }
